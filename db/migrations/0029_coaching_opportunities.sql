...
ALTER TABLE football_runtime.bot_discovery_drafts
    ADD COLUMN coaching_search_details jsonb NOT NULL DEFAULT '{}'::jsonb,
    ADD COLUMN editing_coaching_search_detail text CHECK (
        editing_coaching_search_detail IN (
            'coaching_types',
            'playing_levels',
            'team_formats',
            'schedule',
    ...
    ADD COLUMN coaching_search_detail_draft jsonb NOT NULL DEFAULT '[]'::jsonb,
    ADD COLUMN coaching_search_schedule_prompt text CHECK (
        coaching_search_schedule_prompt IN ('start_local_date')
    );

ALTER TABLE football_runtime.recommendation_completed_searches
    ADD COLUMN coaching_search_details jsonb NOT NULL DEFAULT '{}'::jsonb;

CREATE FUNCTION football_runtime.read_current_opportunity_result_projection(
