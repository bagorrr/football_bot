...
meaning to the candidate target, either `open_match` or `tournament`. A
`tournament` candidate requires event time and exactly one source-bound
`open_participation` or `registration_open` fact; any optional tournament facts
must also be source-bound. Return one outgoing `supports` relation for the root,
