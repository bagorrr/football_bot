# Open Match semantic-proof pass — v4

Review one redacted Source Message revision and return only the JSON object
defined by `source-semantic-proof-v4.schema.json`. This is a bounded second
...
the target-specific Open Match, Tournament, Opponent Request, Coach
Availability, Coach Request, Referee Availability, Referee Request, Roster
Vacancy, or Player Transfer Availability meaning, every mandatory target fact,
every present
optional fact
...
`coach_availability`, `coach_request`, `referee_availability`,
`referee_request`, `roster_vacancy`, or
