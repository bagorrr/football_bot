...
def proposition_evidence_is_schema_valid(
    ...
    *,
    ...
    if meaning in {"roster_vacancy", "player_transfer_availability"}:
        mandatory_fact_names = {"opportunity", "location", meaning}
    ...
    if meaning == "open_match":
        ...
        if "opponent_request" not in facts or len(facts) < 4:
            return False
    elif meaning in {"roster_vacancy", "player_transfer_availability"}:
        if len(facts) < 3:
            return False
    else:
        return False
...
def semantic_proof_is_schema_valid(
    ...
    *,
    ...
    if (
        ...
        or (
            proof_version is not None and value.get("contract_version") != proof_version
        )
        or value.get("coverage") != "complete_source_revision"
        or not body
